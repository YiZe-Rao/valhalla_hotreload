...
使用方法:
    ...
    # 物理机运行:
    python3 realtime_traffic_daemon.py \
        --config /home/admin/valhalla_traffic_poc_/valhalla_tiles/valhalla.json \
...
class RealtimeTrafficUpdater:
    ...
    def _get_traffic_dir(self) -> str:
        ...
        try:
            ...
            # Docker 容器内路径为 /valhalla_tiles，物理机路径根据配置
            return os.path.dirname(traffic_extract) or '/valhalla_tiles'
        except Exception as e:
            logger.warning(f"Failed to read config, using default: {e}")
            return '/valhalla_tiles'
...
def main():
    ...
    parser.add_argument(
        '--config', '-c',
        default='/valhalla_tiles/valhalla.json',
        ...
        for record in stream_heartbeat_csv(args.heartbeat):
            ...
            if time.time() - batch_start >= args.interval:
                ...
                if not args.dry_run:
                    ...
                    # dry-run: 仍然处理数据并显示统计，但不写文件
                    logger.info(f"  [DRY-RUN] Would process {len(batch)} records")
                    # 简单统计
                    valid = sum(1 for r in batch if r.lat > 0 and r.lon > 0)
                    logger.info(f"  [DRY-RUN] Valid records: {valid}")
                    speeds = [r.speed for r in batch if r.speed > 0]
                    if speeds:
                        logger.info(f"  [DRY-RUN] Speed range: {min(speeds):.1f} - {max(speeds):.1f} km/h")
